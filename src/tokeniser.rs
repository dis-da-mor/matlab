...
impl TupleType {
    pub fn new(elems: impl IntoIterator<Item = TokenType>) -> Self {
        Self {
...
impl Token {
    pub fn type_of(&self) -> TokenType {
        match self {
            ...
            Token::Tuple(_) => TokenType::Tuple,
            Token::Matrix(_) => TokenType::Matrix,
            Token::Function(_) => TokenType::Function,
        }
    }
...
impl Tuple {
    ...
    pub fn to_type(&self) -> TupleType {
        TupleType {
            elems: self.elems.iter().map(|token| token.type_of()).collect(),
        }
    }
}
